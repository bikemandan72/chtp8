/*
12.19 (Depth of a Binary Tree)
Write a function depth that receives a binary tree and
determines how many levels it has.
*/
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
unsigned int depthTree(TreeNodePtr *treePtr);
...
{ 
	...
	printf("Depth of the Binary Tree is %u\n", depthTree(&rootPtr));
	freeTree(&rootPtr);
}
unsigned int depthTree(TreeNodePtr *treePtr) {
	static unsigned int depth = 0, maxDepth = 0;
	if (*treePtr != NULL) {
		depth++;
		if (depth > maxDepth) {
			maxDepth = depth;
		}
		depthTree(&((*treePtr)->rightPtr));
		depthTree(&((*treePtr)->leftPtr));
		depth--;
	}
	return maxDepth;
}
